...
import { Plus, Dog, ArrowUp, ArrowDown, ArrowUpDown } from "lucide-react";
...
function ordenNreg(d: DuplaConLibreta) {
  const n = parseInt(d.nreg ?? "", 10);
...
function comparar(a: DuplaConLibreta, b: DuplaConLibreta, columna: ColumnaOrden, asc: boolean) {
  if (columna === "nreg") {
    const an = ordenNreg(a);
    const bn = ordenNreg(b);
...
export function DuplasTabla({ duplas }: { duplas: DuplaConLibreta[] }) {
  ...
  const [orden, setOrden] = useState<{ columna: ColumnaOrden; asc: boolean }>({ columna: "nreg", asc: true });
  ...
  function ordenarPor(columna: ColumnaOrden) {
    setOrden((actual) =>
      actual.columna === columna ? { columna, asc: !actual.asc } : { columna, asc: true }
    );
  }
                ...
                {COLUMNAS.map((c) => (
                  <th key={c.key} className="px-5 py-3 font-semibold">
                    <button
                      ...
                      className="foco inline-flex items-center gap-1 hover:text-slate-900"
                    >
                      {c.etiqueta}
                      {orden.columna === c.key ? (
                        orden.asc ? (
